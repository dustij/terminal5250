...
public class T5250Frame extends JFrame {
    ...
    public T5250Frame(String title) {
        ...
        setPanelContents(new Screen("mainMenu"));
    ...
    public void setPanelContents(Screen screen) {
        screenBuffer.clear();
        for (Position p : screen.getItems()) {
            // TODO
        }
    }
    ...
    private void setupKeyBindings() {
        ...
        actionMap.put("handleTab", new AbstractAction() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                int nextIndex = (panel.getCurrentFieldIndex() + 1) % screenBuffer.getFieldPsitions().size();
                panel.setCurrentFieldIndex(nextIndex);
                int row = screenBuffer.getFieldPositionAt(panel.getCurrentFieldIndex()).getRow();
                int col = screenBuffer.getFieldPositionAt(panel.getCurrentFieldIndex()).getCol();
        ...
        actionMap.put("handleTab", new AbstractAction() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                int nextIndex = Math.abs((panel.getCurrentFieldIndex() - 1))% screenBuffer.getFieldPsitions().size();
                panel.setCurrentFieldIndex(nextIndex);
                int row = screenBuffer.getFieldPositionAt(panel.getCurrentFieldIndex()).getRow();
                int col = screenBuffer.getFieldPositionAt(panel.getCurrentFieldIndex()).getCol();
                cursor.moveTo(row, col);
